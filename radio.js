...
var serverQueue = lista.queue.get(message.guild.id);
    ...
    if(serverQueue) {
        if(serverQueue.music == true) return message.channel.send({
            embed: {
                description: `<a:erro:777261754929381406>| já estou tocando **\`música\`** no servidor. Use **\`${prefixo}stop\`** para parar a **música**`,
                color: 0x36393f,
            }
        })
    }
    ...
    leave(message, serverQueue, dean);
        ...
        if(rad === true) rad = "<a:verm_INS_Verified:770003120163389481> Ativada";
            if(rad === false) rad = "<a:erro:777261754929381406> Desativada";
        }

        } else {
            let = rad;
            rad = "<a:erro:777261754929381406> Desativada";
        }
...
async function join(radioJson, message, args, dean, prefixo) {
    ...
    if (!voiceChannel) {
        message.channel.send({
            embed: {
                description: `<a:erro:777261754929381406> | ${message.author}, você **precisa** está em um canal de voz para ligar a **\`rádio\`**`,
                color: 0x36393f,
            }
        })
    } else {
...
var permissions = voiceChannel.permissionsFor(message.client.user);
    ...
    if (controle == false) {
        message.channel.send({
            embed: {
                description: `<a:erro:777261754929381406> | Nenhuma **\`rádio\`** encontrada, digite **\`${prefixo}radio\`** e consulte as **rádios** disponíveis`,
                color: 0x36393f,
            }
        }) 
    } 
    ...
    if(controle == true) {
        ...
        message.channel.send({
            embed: {
                description: `<a:verm_INS_Verified:770003120163389481> | ${message.author}, **Ligando** a rádio \`${radio.name}\` no canal \`${message.member.voiceChannel.name}\``,
                color: 0x36393f,
            }
        }).then(raP => {
...
async function leave(message, serverQueue, dean) {
        ...
        if (!message.member.voiceChannel) return message.channel.send({
            embed: {
                description: `<a:erro:777261754929381406> | ${message.author}, você não está em um canal de **voz**`,
                color: 0x36393f,
            }
        })
        ...
        else if (voiceChannel !== message.guild.members.get(dean.user.id).voiceChannel) return message.channel.send('<a:erro:777261754929381406> | Você não está no canal que eu **estou** **\`conectado\`**');
    
        if (!serverQueue) {
            message.channel.send({
                embed: {
                    description: `<a:erro:777261754929381406> | ${message.author}, a rádio não está **online** no momento`,
                    color: 0x36393f,
                }
            })
        } else {
        ...
        message.channel.send({
            embed: {
                description: `<a:erro:777261754929381406> | ${message.author}, **desligando** a rádio no canal **\`${message.member.voiceChannel.name}\`**`,
                color: 0x36393f,
            }
        }).then(leaveR => {
