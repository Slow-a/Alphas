...
if(!serverQueue) {
    ...
    .setColor('#36393f')
        .addField(`<:disc:815809341311156264> Lista de Reprodução - **${serverQueue.duraTotal}**`, `${serverQueue.soms.map(song => `**[\`${song.numero}\`] -** ${song.title}`).slice(0,10).join('\n')}\n\n`)
...
var queuee = new Discord.MessageEmbed()
    ...
    .setColor('#36393f')
        .addField(`<:disc:815809341311156264> Lista de Reprodução - **[\`${dT}\`]**`, `${serverQueue.soms.map(song => `**[\`${song.numero}\`] -** ${song.title}`).slice(0,10).join('\n')}\n\n`)
            .setFooter(`Página ${pagina} de ${totalPages}`, message.author.displayAvatarURL)
                .setTimestamp()
                    //.setThumbnail(message.guild.iconURL)
