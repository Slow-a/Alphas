...
exports.run = (client, message, args) => {
  database.Guilds.findOne({ _id: message.guild.id }, async function (err, server) {
    ...
    if (args[0] == "set") {
      ...
      } else if (channel.id == server.logs3.channel) {
        return message.channel.send(
          `${Emojis.Errado} - ${message.author}, o canal inserido é o mesmo setado atualmente, tente novamente.`
        );
      } else {
        message.channel.send(
          `${Emojis.Certo} - ${message.author}, o canal de logs3 foi alterado para **<#${channel.id}>** com sucesso.`
        );
        await database.Guilds.findOneAndUpdate(
          { _id: message.guild.id },
          { $set: { "logs3.channel": channel.id } }
        );
      }
      return;
    }
    if (args[0] == "on") {
      if (server.logs3.channel == "null") {
        return message.channel.send(
          `${Emojis.Errado} - ${message.author}, para ativar seu sitema de logs3, sete um canal primeiro.`
        );
      } else if (server.logs3.status) {
        return message.channel.send(
      ...
      .addFields(
        {
          name: `Canal Setado`,
          value:
            server.logs3.channel == "null"
              ? "Nenhum Canal Setado"
              : `<#${server.logs3.channel}>`,
        },
        {
          name: "Status do Sistema",
          value: `No momento o sistema se encontra **${
            server.logs3.status ? `ativado` : "desativado"
          }**`,
        }
      );
...
exports.help = {
  name: "logs3",
  aliases: ["log"],
  description: "Comando para configurar o canal de logs3 do servidor",
  usage: "<prefix>logs3 <#channel>",
  category: "Config",
};