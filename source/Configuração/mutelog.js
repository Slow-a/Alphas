...
exports.run = (client, message, args) => {
  database.Guilds.findOne({ _id: message.guild.id }, async function (err, server) {
    ...
    if (args[0] == "set") {
      ...
      } else if (channel.id == server.logs4.channel) {
        return message.channel.send(
          `${Emojis.Errado} - ${message.author}, o canal inserido é o mesmo setado atualmente, tente novamente.`
        );
      } else {
        message.channel.send(
          `${Emojis.Certo} - ${message.author}, o canal de logs4 foi alterado para **<#${channel.id}>** com sucesso.`
        );
        await database.Guilds.findOneAndUpdate(
          { _id: message.guild.id },
          { $set: { "logs4.channel": channel.id } }
        );
      }
      return;
    }
    if (args[0] == "on") {
      if (server.logs4.channel == "null") {
        return message.channel.send(
          `${Emojis.Errado} - ${message.author}, para ativar seu sitema de logs4, sete um canal primeiro.`
        );
      } else if (server.logs4.status) {
        return message.channel.send(
      ...
      .addFields(
        {
          name: `Canal Setado`,
          value:
            server.logs4.channel == "null"
              ? "Nenhum Canal Setado"
              : `<#${server.logs4.channel}>`,
        },
        {
          name: "Status do Sistema",
          value: `No momento o sistema se encontra **${
            server.logs4.status ? `ativado` : "desativado"
          }**`,
        }
      );
...
exports.help = {
  name: "logs4",
  aliases: ["log"],
  description: "Comando para configurar o canal de logs4 do servidor",
  usage: "<prefix>logs4 <#channel>",
  category: "Config",
};